...
| `𝕏 @todaygansha · v6.10` | 版本號 |

## 點數字看完整名單（v6.10 新增）

**點面板上「未回關 / 未回跟」的數字**，會彈出完整的名單視窗：

- 每一列是 `@帳號` + **已監測天數**（越久沒回你的排越前面，跟頁面上的排序一致）
...
- **路線 B（點網址自動安裝）** → 不用做事。腳本裡已經寫好更新來源，油猴會**自己定期檢查**，
  有新版時圖示會冒出**紅色的數字**，點開 → 按「**更新 / Update**」就好。
  想立刻檢查：油猴圖示 → 控制台 → 「已安裝腳本 / Installed userscripts」分頁 →
  按上方「**檢查更新 / Check for updates**」。
- **路線 A（手動複製貼上）** → **不會自動更新**（因為沒有更新來源）。
...
𝕏 @todaygansha ｜ GANSHA Mutual Radar v6.10
