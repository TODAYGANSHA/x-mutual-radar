...
三點提醒：
- **① 只支援桌面電腦的網頁版 X**（手機 App 不適用）
- **② 需要你自己捲動頁面**來載入名單，腳本才看得到裡面的帳號
- **③ 所有紀錄只存在你自己的瀏覽器本機**，不會上傳到任何地方
...
| `𝕏 @todaygansha · v6.13` | 版本號 |

## 點數字看完整名單（v6.10 新增，v6.11 加上顯示名稱，v6.12 修復抓取，v6.13 重寫徹底解決）

**點面板上「未回關 / 未回跟」的數字**，會彈出完整的名單視窗：

- 每一列是 **顯示名稱** + `@帳號` + **已監測天數**（越久沒回你的排越前面，跟頁面上的排序一致）
- **顯示名稱**（v6.11 加入，v6.12 修復，v6.13 重寫抓取邏輯徹底解決頭像連結擋路）：掃描當下順手記下來的名字，存在你自己瀏覽器裡。大家認的是名字不是 ID，**一眼就能認出是哪個人**；舊帳號若沒抓到名字，只顯示 `@帳號`，不影響功能
...
1. 進 `x.com` 的 `/following` 或 `/followers`，看**右下角黑框最底下那行**：`𝕏 @todaygansha · v6.13`
2. 按 `F12` 開 Console（主控台），看有沒有 `[GANSHA 雷達] v6.13 已載入` 這行
...
𝕏 @todaygansha ｜ GANSHA Mutual Radar v6.13
