...
### Chrome（要能連上 Google 商店）
1. 打開：https://www.tampermonkey.net/
2. 點網頁上的「Download」→ 選 Chrome 那一版
3. 按「**加到 Chrome**」→「**新增擴充功能**」

> 如果 Chrome 商店一直轉圈打不開，改用 Edge 或 Firefox 會省很多事。
...
### 怎麼確認裝好了
瀏覽器**右上角**應該出現一個**黑白方塊積木圖示**。看不到就點右上角的「拼圖/眼睛」圖示，把它釘選出來。
...
1. 點右上角**油猴的黑白方塊圖示**
2. 選「**控制台**」（或「Dashboard / 儀表板」）
3. 點左上的「**+**」號（新增腳本）
4. 會開出一個編輯視窗，裡面有一堆**範例文字** → 用 `Ctrl + A` 全選 → **按 Delete 全部刪掉**（這一步很重要，要清乾淨）
5. 把第 2 步複製的內容 `Ctrl + V` **貼上**
6. 按 `Ctrl + S`，或點上面的「**檔案 → 儲存**」
7. 關掉這個視窗
...
1. **重新整理頁面**（`Ctrl + R`），等 3 秒
2. 點油猴圖示 → 看「GANSHA · X Mutual Radar」的**開關是不是綠色的**；灰色的話點一下打開
